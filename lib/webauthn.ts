...
export async function webAuthnAuthenticate(): Promise<{ token: string; userId: string; nombre: string; email: string; role: string } | null> {
  try {
    const challengeRes = await apiCall<{ challenge: string; rpId: string }>('webauthn-challenge')
    ...
    const verifyRes = await apiCall<{ token: string; userId: string; nombre: string; email: string; role: string }>(
      'webauthn-verify',
      {
...
export async function webAuthnRegister(userId: string, userName: string, userEmail: string): Promise<boolean> {
  try {
    const challengeRes = await apiCall<{ challenge: string; rpId: string; rpName: string }>('webauthn-challenge')
    ...
    const saveRes = await apiCall('saveWebAuthnCredential', {
      userId,
      credentialId: bufferToBase64(credential.rawId),
      publicKey: bufferToBase64(attestationResponse.getPublicKey() || new ArrayBuffer(0)),
