...
export default function EmpleadoHomePage() {
  ...
  async function loadData() {
    ...
    setLoading(true)
    try {
      const dRes = await apiCall<DashboardHoy>('getDashboardHoy', {}, token)
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        />
        <TarjetaMetrica
          titulo="Total en tanque"
          valor={loading ? '...' : formatLitrosNum(data?.total ?? 0)}
          subtitulo="disponible ahora"
          variant="primary"
          isLoading={loading}
      ...
      {puedeHacer('envios') && (
        <div className="card">
          ...
          {loading ? <div className="skeleton h-16 w-full" /> : (() => {
            ...
            const recibidoHoy = data?.recepcionTotal ?? 0
            const disponible = saldoInicial + recibidoHoy
            const totalEnvio = data?.enviadoHoy ?? 0
            const resto = data?.total ?? 0
            const pct = disponible > 0 ? Math.min(100, (totalEnvio / disponible) * 100) : 0
            return (
              <div className="space-y-2">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-center text-sm">
                  <div>
                    <p className="text-xs text-slate-500">Inicio</p>
                    <p className="font-mono font-semibold">{saldoInicial.toFixed(1)} L</p>
                  </div>
                  <div>
                    <p className="text-xs text-slate-500">Recibido</p>
                    <p className="font-mono font-semibold">{recibidoHoy.toFixed(1)} L</p>
                  </div>
                  <div>
